...
| --- | --- | --- |
| **mock** (default) | `VERIFIER_MOCK=true` *or* no `ANTHROPIC_API_KEY` | Returns a canned `verified=true` verdict so the full session loop runs without Claude. Mirrors the Node `USE_MOCK_VERIFIER` worker shipped in M3. |
